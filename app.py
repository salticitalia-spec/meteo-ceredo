...
<style>
    ...
    .aztec-wrapper {
        position: relative; width: 320px; height: 320px; margin: 40px auto; border-radius: 50%;
    ...
    .digital-clock {
        background: rgba(0,0,0,0.85); padding: 10px 22px; border-radius: 12px; color: white;
        font-family: 'Courier New', monospace; font-size: 32px; font-weight: bold; z-index: 10;
    ...
    .ring-circle { fill: none; stroke-linecap: round; transition: stroke-dashoffset 0.4s ease; }
</style>
    ...
    for i in range(12):
        ...
        if idx < len(fc_data['hourly']['time']):
            with cols[i % 6]:
                ...
                st.markdown(f'<div style="background:#111; border:1px solid #333; border-radius:8px; padding:5px; text-align:center; font-size:10px; color:white;">{t}<br><b>{temp}°</b><br><span style="color:{"#FF3311" if p > 30 else "#00FF00"}">{p}%</span></div>', unsafe_allow_html=True)
...
if hi_data and 'daily' in hi_data:
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=250, margin=dict(l=0, r=0, t=5, b=0), showlegend=False, yaxis=dict(showgrid=False), yaxis2=dict(overlaying="y", side="right", showgrid=False), xaxis=dict(showgrid=False))
    st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

# --- 8. OROLOGIO AZTECO (CORREZIONE SYNTAX) ---
now = datetime.now()
