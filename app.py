...
# --- 2. STILE CSS (CYAN INTENSO & ULTRA THIN) ---
st.markdown("""
<style>
    ...
    /* Titolo: Ciano puro, ultra sottile, con bagliore neon */
    .header-text { 
        color: #00FFFF; 
        font-size: 17px; 
        ...
        margin: 0;
        text-align: center;
        /* Effetto luce intensa */
        text-shadow: 0 0 12px rgba(0, 255, 255, 0.8), 0 0 2px rgba(0, 255, 255, 1);
    }

    .radar-box { position: relative; width: 100%; height: 380px; border-radius: 4px; border: 1px solid #1a1a1a; overflow: hidden; margin-bottom: 30px; }
    .crosshair { position: absolute; top: 50%; left: 50%; width: 30px; height: 30px; border: 0.5px solid rgba(255,0,0,0.5); border-radius: 50%; transform: translate(-50%, -50%); z-index: 10; pointer-events: none; }
    ...
    .aztec-wrapper {
        ...
        filter: grayscale(1) brightness(0.1); display: flex; align-items: center; justify-content: center; border: 1px solid #0f0f0f;
    }
    
    .digital-clock {
        background: transparent; color: #fff;
        font-family: 'Inter', sans-serif; font-size: 28px; font-weight: 100; z-index: 5;
    ...
    .aztec-day { color: #333; font-size: 11px; font-weight: 100; letter-spacing: 6px; text-transform: uppercase; }
    
    .xiuh-box { 
        text-align: center; margin: 20px auto; padding: 15px; border-top: 1px solid #111; 
        max-width: 200px;
    }
    .xiuh-days { color: #200; font-family: 'Inter', sans-serif; font-size: 16px; font-weight: 100; letter-spacing: 4px; }
</style>
...
# Intestazione con Ciano Intenso
st.markdown(f"""
<div class="header-container">
    <svg class="logo-svg" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r="48" fill="none" stroke="#00FFFF" stroke-width="0.8" stroke-dasharray="1 5"/>
        <path d="M50 10 L50 40 M50 60 L50 90 M10 50 L40 50 M60 50 L90 50" stroke="#00FFFF" stroke-width="1"/>
...
<div class="aztec-wrapper">
    <div class="digital-clock">{now.strftime("%H:%M")}<span style="color:#00FFFF; font-size:14px; opacity:0.4;">:{s:02d}</span></div>
    <svg class="rings-svg" viewBox="0 0 100 100">
        <circle class="ring-circle" cx="50" cy="50" r="46" stroke="#00FFFF" stroke-width="0.4" stroke-dasharray="289.02" stroke-dashoffset="{off_h}" opacity="0.15"/>
        <circle class="ring-circle" cx="50" cy="50" r="40" stroke="#00FFFF" stroke-width="0.4" stroke-dasharray="251.32" stroke-dashoffset="{off_m}" opacity="0.25"/>
        <circle class="ring-circle" cx="50" cy="50" r="34" stroke="#00FFFF" stroke-width="0.4" stroke-dasharray="213.62" stroke-dashoffset="{off_s}" opacity="0.4"/>
    </svg>
</div>

<div class="aztec-info">
    <div class="aztec-day">{day_lab}</div>
    <div style="color:#222; font-size:9px; font-family:Inter; letter-spacing:3px; margin-top:5px;">{month_lab.upper()} | {year_lab.upper()}</div>
</div>
