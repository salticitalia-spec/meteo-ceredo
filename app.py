...
# --- 3. CSS (Linee corte per evitare SyntaxError) ---
style_css = "<style>"
...
style_css += ".rain-info-box { border:1px solid #FF3311; border-radius:10px; padding:10px; margin:15px 0; font-weight:bold; font-size:16px; }"
style_css += "</style>"
...
if dfc['current_weather']['weathercode'] in [51, 53, 55, 61, 63, 65, 80, 81, 82]:
    ...
    for t, p in zip(h_times, h_prec):
        ...
        if dt_t > now and p > 0.1:
            ...
            msg_pioggia = f"🌧️ INIZIO PIOGGIA: {giorno} {ora}"
            colore_msg = "#FF3311"
            break
...
<div class="main-card">
    ...
    <div style="margin:20px 0;">{get_weather_icon(curr['weathercode'])}</div>
